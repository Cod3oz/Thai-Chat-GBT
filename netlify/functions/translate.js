...
export async function handler(event) {
  try {
    ...
    // Decide source/target languages
    ...
    const genderNote =
      gender === "male"
        ? "Assume the speaker is male; use polite particles like ครับ/นะครับ when needed."
        : gender === "female"
        ? "Assume the speaker is female; use polite particles like ค่ะ/นะคะ when needed."
...
Output STRICT JSON with keys: literal, natural, creative, joking, romantic. No commentary.`;
    ...
    try {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: "Bad JSON from model", raw: data.choices[0].message.content }),
      };
    }
